...
export default function ConfigEditor(props: { profile: string }) {
  ...
  const [configs, { refetch: refreshConfigs }] = createResource(() => scanModConfigs(props.profile));

  const [currentConfig, { refetch: refreshConfig }] = createResource(currentFile, (currentFile) => readModConfig(props.profile, currentFile));
