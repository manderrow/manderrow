...
use crate::mods::{ModMetadata, ModVersion, Version};
use crate::{CommandError, Reqwest};

use super::mod_index::ModId;
use super::profiles::profile_path;
...
pub async fn import_modpack_from_thunderstore_code(
    ...
    let profile_id = match profile_id {
        ...
        None => {
            crate::commands::profiles::create_profile(
                game.into(),
                profile.manifest.profile_name.into(),
            )
            .await?
        }
    };
    ...
    for m in &profile.manifest.mods {
        ...
        mod_id_set.insert(ModId {
            owner: m.full_name.namespace(),
            name: m.full_name.name(),
        });

        let mod_index = crate::commands::mod_index::read_mod_index(game).await?;

        let buf =
            crate::commands::mod_index::get_from_mod_index_impl(&mod_index, &mod_id_set).await?;
        ...
        crate::commands::profiles::install_profile_mod(
            reqwest.clone(),
            profile_id,
            // this is kinda gross
            ModMetadata {
    ...
    tokio::task::spawn_blocking(move || {
        ...
        rt.block_on(local_set.run_until(async move {
            let mut archive = profile.archive;
            for i in 0..archive.len() {
                let mut target_path = profile_path.clone();
                archive = tokio::task::spawn_local(async move {
                    loop {
                        let file = archive
                            .by_index(i)
                            .context("Failed to open file in archive")?;
